...
public class UserServiceTest {
    ...
    public void find_by_username_should_return_found_user_with_existing_username() {
        ...
        // When
        Mockito.when(userRepository.findByUsernameIgnoreCase(Mockito.isA(String.class))).thenAnswer(answer -> {
    ...
    public void get_profile_should_return_profile_for_existing_user_and_public_profile() {
        ...
        // When
        Mockito.when(userRepository.findByUsernameIgnoreCase(username)).thenAnswer(answer -> {
    ...
    public void is_viewable_should_return_true_with_public_profile() {
        ...
        // When
        Mockito.when(userRepository.findByUsernameIgnoreCase(targetedUser)).thenAnswer(answer -> {
            var user = new UserEntity();
            user.setId(1L);
    ...
    @DisplayName("UserService#isViewable should return false with mismatching usernames and private profile")
    @Test
    ...
    public void is_viewable_should_fail_with_unknown_targeted_user() {
        ...
        // When
        Mockito.when(userRepository.findByUsernameIgnoreCase(targetedUser)).thenReturn(Optional.empty());
        ThrowableAssert.ThrowingCallable action = () -> userService.isViewable(targetedUser, currentUser);
    ...
    public void modify_password_should_pass_with_valid_credentials_and_valid_input() {
        ...
        // When
        Mockito.when(userRepository.findByUsernameIgnoreCase(Mockito.anyString())).thenAnswer(answer -> {
            if (!"Eikjos_TV".equalsIgnoreCase(answer.getArgument(0, String.class))) return Optional.empty();
            var entity = new UserEntity();
            entity.setId(1L);
        ...
        Mockito.when(userRepository.save(Mockito.isA(UserEntity.class))).thenAnswer(answer -> answer.getArgument(0, UserEntity.class));
    ...
    public void register_should_return_registered_user_with_valid_input() {
        Mockito.when(userRepository.save(Mockito.isA(UserEntity.class))).thenAnswer(answer -> {
    ...
    public void register_should_fail_with_an_existing_username() {
        Mockito.when(userRepository.existsByUsernameIgnoreCase(Mockito.isA(String.class))).thenReturn(true);
