...
class PushSubscriptionServiceTest {
    ...
    @DisplayName("PushSubscriptionService#sendTo should delete subscription when provider failed to send")
    @Test
    void sendTo_should_delete_subscription_when_provider_failed_to_send() throws PushProvider.PushMessageException {
        // Given
        var target = new UserEntity()
                .setUsername("hello");
        var subscriptions = List.of(
        ...
        when(userRepository.findByUsernameIgnoreCase(target.getUsername())).thenReturn(Optional.of(target));
        when(pushSubscriptionRepository.findAllByUser(target)).thenReturn(subscriptions);
        doThrow(new PushProvider.PushMessageException()).when(pushProvider).send(isA(PushProvider.PushMessage.class));
        pushSubscriptionService.sendTo(target.getUsername(), new PushNotificationDto().setCode("AAAA"));

        // Then
        verify(pushProvider, times(subscriptions.size())).send(isA(PushProvider.PushMessage.class));
        verify(pushSubscriptionRepository, times(subscriptions.size())).delete(isA(PushSubscriptionEntity.class));
    }
    ...
    void subscribe_should_subscribe_user() {
        ...
        var subscription = new SubscribeNotificationsDto()
                .setAuth("a")
                .setP256dh("b")
                .setEndpoint("c");
